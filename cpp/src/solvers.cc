...
int relpose_nullspace(const Eigen::Matrix<double,9, 4> &basis, std::vector<Eigen::Matrix3d> *essential_matrices) {
    Eigen::Matrix<double, 4, 9> N = basis.transpose();
...
int relpose_5pt(const std::vector<Eigen::Vector3d> &x1, const std::vector<Eigen::Vector3d> &x2,
    ...
    return relpose_nullspace(N, essential_matrices);
}
...
int relpose_sum(const Eigen::Matrix<double, 9, 9> &M,
    ...
    Eigen::JacobiSVD<Eigen::Matrix<double,9,9>> svd(M, Eigen::ComputeFullV);
    Eigen::Matrix<double, 9, 4> N = svd.matrixV().rightCols(4);

    std::vector<Eigen::Matrix3d> essential_matrices;
    int n_sols = relpose_nullspace(N, &essential_matrices);

    output->clear();
    output->reserve(n_sols);
    for (int i = 0; i < n_sols; ++i) {
        // The M matrix is orderd such that we get the transpose 
        motion_from_essential(essential_matrices[i], {x01.normalized()}, {x02.normalized()}, output);
    }
